extern crate core;

use stellar_xdr::Error;
use rs_sb_history_verification::hash_transaction_history_result_entry;

#[test]
fn test(){
    let xdr = "AAh/CAAAAAEGJh/ut6Pw5WiDtPWF5h94fONDaUn+YwXn7Wdt5pFAogAAAAAAAABkAAAAAAAAAAEAAAAAAAAAAAAAAAAAAAAAAAAAAA==";
    let bytes = base64::decode(xdr).unwrap();
    let result = hash_transaction_history_result_entry(bytes).unwrap();
    assert_eq!(result.ledger, 556808);
    assert_eq!(result.hash, [197, 223, 135, 233, 220, 177, 122, 123, 25, 178, 71, 248, 30, 27, 251, 57, 15, 1, 7, 161, 220, 118, 242, 129, 67, 187, 32, 207, 36, 163, 233, 59]);
}

#[test]
fn test_xdr_not_transaction_history_result_entry(){
    let xdr = "";
    let bytes = base64::decode(xdr).unwrap();
    let result = hash_transaction_history_result_entry(bytes);
    assert_eq!(Err(Error::Invalid),result)
}